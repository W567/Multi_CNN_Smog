...
result = pretreat_data(0,6,0,6)
data_cd = result
ckpt_dir = '/home/wu/ml_workspace/Multi_CNN_smog/model/label_shuffle'
def conv2d(x,w): 
...
x=tf.placeholder(tf.float32,[None,16,80,90]) 
y_=tf.placeholder(tf.float32,[None,6]) 
x_img = tf.transpose(x,[0,2,3,1])

w_conv1=tf.Variable(tf.truncated_normal([5,5,16,32],stddev=0.1)) 
...
w_conv3=tf.Variable(tf.truncated_normal([3,3,64,80],stddev=0.1)) 
b_conv3=tf.Variable(tf.constant(0.1,shape=[80])) 
h_conv3=tf.nn.relu(conv2d(h_pool2,w_conv3)+b_conv3) 
h_pool3=max_pool_2x2(h_conv3) 

w_conv4=tf.Variable(tf.truncated_normal([3,3,80,128],stddev=0.1)) 
